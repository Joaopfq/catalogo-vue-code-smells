...
| Any type                                                          | Uso do tipo "Any"                                                                                |
| HTTP calls inside components                                      | Usar chamadas HTTP diretamente nos componentes                                                   |
| Reference child component with watcher                            | Referenciar um componente filho através do uso de um prop/watcher                                |
| Omitting the Key Directive on v-for                               | Omitir a diretiva da chave do v-for                                                              |
| Unintentionally Mutating Props                                    | Mudar uma prop diretamente ao invês de usar emits para muda-la                                   |
...
| Uncontrolled Components                                           | Componente que não usa props/state para manipular dados do formulário                            |
| Enum Implicit Values                                              | Usar enums sem definir valores explicitos para eles                                              |
...
| Child component alters parent state directly                      | Componente filho alterar estado do pai diretamente                                               |
